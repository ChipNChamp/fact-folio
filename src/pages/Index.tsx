        ...
        <ScrollArea className="flex-1">
          <main className="flex-1 flex flex-col px-4 py-4 max-w-4xl mx-auto w-full">
            <section className="mb-6 animate-slide-in">
              <Button size="wide" variant="pass" className="relative overflow-hidden group flex items-center justify-center h-14 w-full" onClick={() => navigate("/lessons")}>
            ...
            <section className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6 animate-slide-in">
              {categories.map(category => <div key={category.name} className="flex flex-col space-y-1">
                  <button onClick={() => navigate(category.path)} className={`rounded-xl p-3 flex flex-col items-center justify-center text-center h-28 transition-all duration-200 border border-border/30 ${category.color} shadow-sm hover:shadow-md hover:scale-[1.02]`}>
                    {category.icon}
                    <span className="font-medium">{category.name}</span>
                    {showShortcuts && <span className="keyboard-shortcut mt-1">{category.shortcut}</span>}
                  </button>
                  <button onClick={() => navigate(category.reviewPath)} className={`rounded-lg p-1 flex items-center justify-center text-center text-xs font-medium transition-all duration-200 border border-border/30 ${category.color} bg-opacity-30 hover:bg-opacity-50`}>
                    Review
                  </button>
                </div>)}
            </section>
            ...
            <section className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
              <Button size="wide" className="relative overflow-hidden group h-14" onClick={() => navigate("/review")} disabled={entryCount === 0}>
                <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                Omni-Review
              </Button>
              
              <Button size="wide" variant="outline" className="relative overflow-hidden group flex items-center justify-center h-14" onClick={() => navigate("/entries")}>
                <BookOpen className="h-5 w-5 mr-2" />
                All Entries
              </Button>
            </section>
            
            {entryCount === 0 && <p className="text-sm text-muted-foreground mt-3 text-center">
                Add some entries to begin reviewing
              </p>}
            
            {showShortcuts && <div className="mt-2 mb-6 text-xs text-muted-foreground text-center animate-fade-in">
